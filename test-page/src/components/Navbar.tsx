...
export default function Navbar() {
  ...
  <div className="flex items-center justify-between">
      ...
      {/* Car Selection Box */}
      <div className="relative w-full lg:max-w-3xl lg:mx-auto mt-3 px-4" ref={dropdownRef}>
        <motion.div 
        ...
        <AnimatePresence>
          {isDropdownOpen && (
            <motion.div 
              className="absolute top-full left-4 right-4 mt-2 bg-[#FFFFFF] border border-gray-200 rounded-2xl shadow-lg z-50 max-h-96 overflow-hidden"
                  ...
                  filteredCars.map((car) => (
                    <motion.div
                      ...
                      className={`p-4 hover:bg-gray-50 cursor-pointer border-b border-gray-100 last:border-b-0 ${
                        car.id === currentCar.id ? 'bg-red-50 border-red-200' : ''
                      }`}
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="font-medium text-black">{car.title}</h4>
                          <p className="text-sm text-gray-600">{car.subtitle}</p>
                          <div className="flex gap-2 mt-1">
                            <span className="text-xs bg-[#F5F5F5] text-black px-2 py-1 rounded">
                              {car.year}
                            </span>
                            <span className="text-xs bg-[#F5F5F5] text-black px-2 py-1 rounded">
                              {car.make}
